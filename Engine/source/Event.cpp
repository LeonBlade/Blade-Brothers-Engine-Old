...
void Event::OnEvent(SDL_Event* event)
{
	switch (event->type) 
	{
		...
		case SDL_MOUSEBUTTONDOWN:
			switch (event->button.button)
			{
				case SDL_BUTTON_LEFT:
					OnLButtonDown(Vector2(event->button.x, 
										  event->button.y));
					break;
				case SDL_BUTTON_RIGHT:
					OnRButtonDown(Vector2(event->button.x, 
										  event->button.y));
					break;
				case SDL_BUTTON_MIDDLE:
					OnMButtonDown(Vector2(event->button.x, 
										  event->button.y));
					break;
			}
			break;
		...
		case SDL_MOUSEBUTTONUP:
			...
			{
				case SDL_BUTTON_LEFT:
					OnLButtonUp(Vector2(event->button.x, 
										event->button.y));
					break;
				case SDL_BUTTON_RIGHT:
					OnRButtonUp(Vector2(event->button.x, 
										event->button.y));
					break;
				case SDL_BUTTON_MIDDLE:
					OnMButtonUp(Vector2(event->button.x, 
										event->button.y));
					break;
			}
			break;
		...
		case SDL_JOYAXISMOTION:
			OnJoyAxis(event->jaxis.which, 
					  event->jaxis.axis, 
					  event->jaxis.value);
			break;
			
		case SDL_JOYBALLMOTION:
			OnJoyBall(event->jball.which, 
					  event->jball.ball, 
					  event->jball.xrel, 
					  event->jball.yrel);
			break;
		...
		case SDL_JOYHATMOTION:
			OnJoyHat(event->jhat.which, 
					 event->jhat.hat, 
					 event->jhat.value);
			break;
			
		case SDL_JOYBUTTONDOWN:
			OnJoyButtonDown(event->jbutton.which, 
							event->jbutton.button);
			break;
			
		case SDL_JOYBUTTONUP:
			OnJoyButtonUp(event->jbutton.which, 
						  event->jbutton.button);
			break;
...
void Event::OnMouseMove(Vector2 m, Vector2 r, bool left,bool right,bool middle) 
{
    //Pure virtual, do nothing
}
