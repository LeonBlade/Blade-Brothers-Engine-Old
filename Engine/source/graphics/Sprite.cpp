...
bool Sprite::OnDraw(SpriteTexture texture, Vector2 position, SDL_Rect tile, Vector2 size)
{
	...
	glBegin(GL_QUADS);
	{
		...
		glVertex3i(position.x, position.y, 1);
		glTexCoord2f(max_x, max_y);
		glVertex3i(position.x + tile.w + size.x, position.y, 1);
		glTexCoord2f(max_x, min_y);
		glVertex3i(position.x + tile.w + size.x, position.y + tile.h + size.y, 1);
		glTexCoord2f(min_x, min_y);
		glVertex3i(position.x, position.y + tile.h + size.y, 1);
	}
	glEnd();
